...
public:
	void AddTimer(function<void()> cb, TimeStamp when, double interval, Timer*& timer, CLogFile& logfile); // 添加Timer的函数，在insert上包装，不必加锁，只会被拥有TimerContainer的IO线程调用
	void CancelTimer(Timer* timer, CLogfile& logfile); // 不必加锁，只会被拥有TimerContainer的IO线程调用
